<?php
/**
*Plugin Name: SriToni Razorpay
*Plugin URI:
*Description: SriToni Admin interface to Razorpay Smart Collect Virtual accounts
*Version: 2019031900
*Author: Madhu Avasarala
*Author URI: http://sritoni.org
*Text Domain: sritoni_razorpay
*Domain Path:
*/
defined( 'ABSPATH' ) or die( 'No script kiddies please!' );

require_once(__DIR__."/sritoni_razorpay_api.php");  	// my Razorpay Api class
require_once(__DIR__."/MoodleRest.php");   				// Moodle REST API driver for PHP
require_once(__DIR__."/sritoni_razorpay_settings.php"); // file containing class for settings submenu and page
...
if ( is_admin() ){ // admin actions
  ...
  // Now add a new submenu for sritoni razorpay plugin settings in Woocommerce. This is to be done only once!!!!
  $sritoniRazorPaySettings = new sritoni_razorpay_settings();
}
...
function init_custom_gateway_class()
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Bank Transfer to RzpVA', 'woocommerce' );
		$this->method_description = __( 'BACS to Individual Razorpay Virtual Account. More commonly known as direct bank transfer', 'woocommerce' );
		// Load the settings.
	...
	private function bank_details( $order_id = '' ) {
		...
		// get user meta bank details for associated razorpay VA
...
function VA_payments_callback()
{
	...
	$razorpay_api 	= new sritoni_razorpay_api; // new API object
	//
	$payments	= $razorpay_api->getPayments($va_id)->items;	// list of payments msde into this VA
	//
	foreach ($payments as $key=> $payment)
		{
			...
			$payment_amount		= 0.01 * $payment->amount;	// in ruppees
...
function set_orders_newcolumn_values($colname)
{
	...
	{ 
		...
		case ( ($reconcilable == true) && ($reconcile == 1) ) :
			...
			$payment_details		= $razorpay_api->getPaymentDetails($payment_id);
	...
	if ( 'VAid' === $colname  )
	{
		...
		{
			case ($payment_method == "vabacs") :
				//$link_address = 'https://dashboard.razorpay.com/#/app/virtualaccounts/' . $va_id;
				$data = array(
								"va_id"			=>	$va_id,
...
function reconcile_ma($order, $payment, $payment_details, $reconcile, $reconcilable)
{
	...
	$payment_datetime		= new DateTime('@' . $payment->created_at);
	$payment_datetime->setTimezone($timezone);
	$order_note = 'Payment received by Razorpay Virtual Account ID: ' . get_post_meta($order->id, 'va_id', true) .
